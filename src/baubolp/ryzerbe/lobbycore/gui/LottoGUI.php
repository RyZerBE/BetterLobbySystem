...
class LottoGUI {
    ...
    public function send(): void{
        $player = $this->player;
        $inv = InvMenu::create(InvMenu::TYPE_DOUBLE_CHEST)->setName(Loader::PREFIX . TextFormat::AQUA . "500.000 Coins")->setListener(function(InvMenuTransaction $transaction) use ($player): InvMenuTransactionResult{
                $item = $transaction->getItemClicked();
                $action = $transaction->getAction();
                if($item->getId() === Item::PAPER){
                    for($i = 0; $i < 54; $i++) $transaction->getAction()->getInventory()->setItem($i, Item::get(Item::ENDER_CHEST)->setCustomName(TextFormat::GOLD . "???"));
                    for($i = 0; $i < 10; $i++) $player->playSound("random.pop", 5.0, 1.0, [$player]);
                ...
                if(($obj = LobbyPlayerCache::getLobbyPlayer($player)) != null){
                    ...
                    if(count($obj->getLottoWin()) > 4){
                        ...
                        $coins = 0;
                        foreach($obj->getLottoWin() as $win) $coins += $win;
                        ...
                        if($coins > 5000 && $coins < 25000){
                            foreach(Server::getInstance()->getOnlinePlayers() as $players) $players->sendMessage("\n\n" . Loader::PREFIX . LanguageProvider::getMessageContainer('lobby-lotto-win', $players->getName(), [
                                    '#coins' => $coins,
        ...
        for($i = 0; $i < 54; $i++) $inv->getInventory()->setItem($i, Item::get(Item::ENDER_CHEST)->setCustomName(TextFormat::GOLD . "???"));
        $inv->send($player);
    }
}