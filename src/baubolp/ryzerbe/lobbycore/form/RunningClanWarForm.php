...
class RunningClanWarForm
{
    ...
    public static function open(Player $player)
    {
        ...
        foreach (RunningClanWarProvider::getRunningClanWars() as $clanWarMatch)
            $form->addButton(str_replace("%clan1%", $clanWarMatch->getClan1(),
                    str_replace("%clan2%", $clanWarMatch->getClan2(),
                        str_replace("%map%", $clanWarMatch->getMap(),
                            str_replace("%elo%", ($clanWarMatch->eloEnabled() === true) ? TextFormat::BOLD . TextFormat::GREEN . "ON" : TextFormat::RED . TextFormat::BOLD . "OFF", self::BUTTON_TEXT))))
                , -1, "", $clanWarMatch->getServer());
